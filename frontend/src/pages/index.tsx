...
export default function IcConnectPage() {
  ...
  const HomeCanister = useCandidActor<CandidActors>("home", currentIdentity, {
  ...
  async function getProfile() {
    try {
      setLoading(true);
      const response = await HomeCanister.getProfile();
      setProfile(response);
      setLoading(false);
    } catch (error) {
      console.error({ error });
  ...
  async function createUser() {
    try {
      ...
      const response = await HomeCanister.createProfile(newUser);
  ...
  async function changeUserState(newState: State, userPrincipal: Principal) {
    try {
      setLoading(true);
      const response = await HomeCanister.changeUserState(newState, userPrincipal);
  ...
  async function deleteUser(userPrincipal: Principal) {
    try {
      setLoading(true);
      const response = await HomeCanister.deleteUser(userPrincipal);
  ...
  async function getAllProfiles() {
    try {
      setLoading(true);
      const profiles = await HomeCanister.getAllProfiles();
