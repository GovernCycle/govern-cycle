...
const UbicationDropdown: React.FC<UbicationDropdownProps> = ({ options, placeholder }) => {
  ...
  return (
    <Menu as="div" className="relative">
      <MenuButton className="group flex items-center justify-between h-full px-3 py-2.5 text-sm font-semibold text-text-tertiary outline-none drop-shadow-[-4px_-4px_6px_rgba(255, 254, 239,0.2)] duration-200 ease-in-out hover:text-charcoal-500">
        <span>{selectedOption || placeholder}</span> {/* Esto o puse para mostrar el placeholder si no se ha seleccionado ninguna opción */}
      ...
      >
        <Menu.Items className="absolute z-50 mt-4 w-full rounded-md bg-tan-500/70 shadow-lg">
          {options.map((option) => (
