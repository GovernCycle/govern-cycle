...
shared actor class Dip721NFT(custodian: Principal, init : Types.Dip721NonFungibleToken) = Self {
  ...
  public query func balanceOfDip721(temp: Text) : async Nat64 {
    let user : Principal = Principal.fromText(temp);
  ...
  public shared func transferFromDip721(from: Principal, to: Principal, token_id: Types.TokenId) : async Types.TxReceipt {
    let addressAdmin : Principal = Principal.fromText("sfbeo-fbfg6-pcizc-usivn-h4myv-ns6bu-vcdkj-ns6oq-ewc26-c4sxd-hqe");
    
    return transferFrom(from, to, token_id, addressAdmin);
  };
 ...
 // Define a public function that queries and returns the NFT's logo:
  ...
  public shared func mintDip721(to: Principal, metadata: Types.MetadataDesc) : async Types.MintReceipt {
    ...
    let addressAdmin : Principal = Principal.fromText("2fb3e-yw464-kqobv-62rnv-m3ezh-pobeu-cz3xv-7xpok-qyb4f-qcttt-uae");
    
    if (not List.some(custodians, func (custodian : Principal) : Bool { custodian == addressAdmin })) {
      return #Err(#Unauthorized);
    };
